<script lang="ts">
	...
	async function submitChallenge() {
		...
		status = 'submitting';
		const { addChallenge } = await import('$lib/firebase');
		addChallenge(
			{
				title: data.title,
...
<h1>
	...
	<span> = </span>
	<span class={status === 'success' ? 'text-green-500' : status === 'error' ? 'text-red-600' : ''}
		>"{status}"</span
	>
</h1>
...
>
	<span class="text-red-500 font-bold">submitChallenge</span>(<span>challenge</span>);
</button>
